...
- 文件标签通过“选择脚本连接”明确选择已有 PostgreSQL / Oracle 目标，不跟随左侧连接变化。选择本身不连接、不读取元数据、不执行 SQL；取消保留原目标。首次执行或会话操作前可以更换，准入后锁定目标及配置快照，并沿用原有风险确认、只读及事务规则。首次准入前目标被删除或改成其他类型时拒绝执行，不能按同名连接替代；选择不会修改或保存 SQL 文件。
