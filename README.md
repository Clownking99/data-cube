...
- **多语句执行详情**：批量结果分别统计正常返回、失败、超时和取消；选择摘要后点击“执行详情”或按 `Enter`，只读查看对应 SQL、错误、耗时及已加载/影响行数。排序后仍对应原语句编号；新结果或关闭会清除旧详情。不重新执行，不保留查询数据集，不表示事务已提交或回滚。最多显示前 1,000 条摘要，单段 SQL/错误最多 16,384 UTF-16 单元，合计文本最多 1,048,576 单元；超限明确提示省略。
