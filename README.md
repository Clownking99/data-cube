...
朝花夕拾/
├── src/com/datacube/
│   ├── DataCube.java           # 主类：CLI入口、连接管理、菜单循环
│   ├── ConsoleLogger.java      # 控制台日志：时间戳、进度条、摘要、文件日志
│   ├── OracleExporter.java     # Oracle导出：DDL导出 + 多线程数据导出
│   ├── PgImporter.java         # PostgreSQL导入：完整/增量模式
│   ├── PgVerifier.java         # PostgreSQL验证：表数/行数/序列/函数检查
│   ├── TypeConverter.java      # 类型转换：Oracle→PG类型映射
│   ├── SqlUtils.java           # SQL工具：INSERT生成、值转义、SQL分割
│   └── ColumnInfo.java         # 列信息数据类
├── DataCube.jar                # 可执行 JAR
├── build.sh                    # 编译打包脚本
├── run.sh                      # 运行脚本
├── drivers/                    # JDBC 驱动（已内置）
