...
**结论：本地完整单测、免安装包构建、隔离首次启动和合成旧配置重启保留已通过；范围切换/弹窗键盘等桌面验收仍有工具限制，并发现查询结果没有隐藏列入口，不能发布。** 本轮不修改生产代码、测试代码或正式构建配置；不推送、不打 tag、不安装/卸载软件。
...
| 隐藏列输出 | UI 入口缺失；内部契约另有自动化覆盖 | 先确认是否增加用户入口，再做真实隐藏与文件验收；不靠夹具替代 |
...
恢复工作时先核对工作树、构建基线及隔离 cfg，不要直接启动用户已安装实例。若重新构建覆盖了生成 cfg，必须在 GUI 启动前重新设置独占 `user.home`。独立弹窗可可靠定位后再补验相应条目；隐藏列需先确认产品范围。期间使用合成数据，不访问 `.testagent/`、公司数据库、真实历史或凭据。当前分支和本地产物暂保留供续验，P0.2 未标记完成。
...
- 由于阶段验收尚未完成，独立分支及隔离产物暂保留，尚未合并回 `main`。确认隐藏列范围并补齐剩余验收后，再按用户授权本地合并；不自动扩大为推送、安装升级或发布。P1 草稿保存规则仍待设计确认，本轮没有实施。
