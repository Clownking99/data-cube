...
Following tests must use actual ContentTabPane ownership: normal-editor duplicate focus, recovered duplicate focus, refused close mapping retention, approved finalization removal, factory/installation failure abort and subscription release. Source and runtime verification on2026-08-31 corrected the earlier failure-injection assumption: a factory starting `closeAllManagedTabsMandatory()` before returning its pane does NOT refuse an already-acquired reservation. `Reservation.register` deliberately permits it, then reservation release starts the close barrier. Test this as successful installation followed by completed close and released bindings. Separately supply a throwing `SingleSelectionModel<Tab>` to the actual TabPane so selection fails during ManagedTabInstaller after draft binding; wait for its abort barrier and check no remaining handle or installed mapping. Neither case needs a production-only fault hook.
