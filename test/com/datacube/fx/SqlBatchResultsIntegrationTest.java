    ...
    void resultSwitcherAndFailureSummaryStayInsideNarrowAndWidePanels(double width, String theme) throws Exception {
        try (var f = fixture()) {
            FxUiTestSupport.call(() -> {
                ...
                for (String id : List.of("sql-batch-choice", "sql-batch-summary", "sql-batch-details", "sql-batch-next-failure")) {
                    ...
                    if (node instanceof javafx.scene.control.Button) assertTrue(node.getWidth() + 1 >= node.prefWidth(-1), id);
                }
