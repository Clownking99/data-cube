...
public final class PgMetadataReader implements MetadataReader {
    ...
    public List<TableInfo> tableAndViewNames(String schema, int maxRows) throws SQLException {
        ...
        String sql = "SELECT table_name, table_type FROM information_schema.tables "
                + "WHERE table_schema = ? AND table_type IN ('BASE TABLE','VIEW') ORDER BY table_name, table_type";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, schema);
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (out.size() < maxRows && rs.next()) {
                    ...
                    if (!"BASE TABLE".equals(type) && !"VIEW".equals(type)) throw new SQLException("Unexpected object type");
